...
exports.resolvers = {
	...
	async getUserGroup({Id, slug}, {Server}) {
		// Id take precedence
		if (Id) {
	...
	async setUserGroup({input}, {Server}) {
	...
	async deleteUserGroup({Id, action}, {Server}) {
		const [err] = await Server.delete_user_group(Id, action);

		return err||true;
	},

	getUserGroups: (__, {Server}) => Server.get_user_groups()
};
